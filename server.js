  ...
  socket.on("sendVideo", (data) => {
    ...
    socket.on("video", (chunk) => {
      if (chunk.arrayBuffer) {
        chunk.arrayBuffer.map((stream)=>{
          fileStream.write(stream,(err) => {
            if (err) {
              console.error("❌ Erreur lors de l'écriture dans le fichier :", err);
            } else {
              console.log("✅ Chunk écrit avec succès");
            }
          })
        })
      }
    ...
    socket.on("disconnect", () => {
      ...
      if (clientStreams[filename]) {
        clientStreams[filename].end(); // Fermer le stream proprement
       delete clientStreams[filename]; // Supprimer du cache
      }
    });
  });
